...
# Похідний клас "Екстремальний маршрут" з використанням super()
class ExtremeRoute(TourRoute):
    def __init__(self, name, country, duration, difficulty, obstacles):
        # Виклик конструктора базового класу через super()
        super().__init__(name, country, duration, difficulty)
...
extreme_route_super = ExtremeRoute("Альпи", "Швейцарія", 7, "середній", 3)
print(extreme_route_super.get_properties())
print(f"Кількість перешкод: {extreme_route_super.get_obstacles()}")
print(f"Вартість екстремального маршруту: {extreme_route_super.calculate_cost()} грн")
